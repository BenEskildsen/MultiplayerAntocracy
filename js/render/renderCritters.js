...
const {renderAntSprite} = require('./renderAnt');
const {
  renderWorm, renderCentipede,
  renderCaterpillar,
  renderDeadCaterpillar, renderDeadCentipede, renderDeadWorm,
...
const renderCritter = (ctx, game, critter): void => {
  let sprite = null;
  let isDead = false;
  ...
  if (sprite == null || sprite.img == null) return;
	ctx.save();
	ctx.translate(
    critter.position.x + critter.width / 2,
    critter.position.y + critter.height / 2,
  );
  ...
  ctx.drawImage(
    ...
    0, 0, critter.width, critter.height,
  );
  ...
  if (!isDead && critter.hp < game.config[critter.type].hp) {
    renderHealthBar(ctx, {...critter}, game.config[critter.type].hp);
  }
