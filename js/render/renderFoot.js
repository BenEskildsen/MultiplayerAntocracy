...
const {getFootSprite, getInterpolatedSize} = require('../selectors/sprites');

const renderFoot = (ctx, game, foot): void => {
  switch (foot.state) {
    case 'lifted':
...
const renderFallingFoot = (ctx, game, foot): void => {
  ctx.save();
  ctx.fillStyle = 'black';
  ...
  ctx.fillRect(
    foot.position.x + (width - size.x) / 2,
    foot.position.y + (height - size.y) / 2,
    size.x, size.y,
  );
  ctx.restore();
};

const renderStompingFoot = (ctx, game, foot): void => {
  // ctx.save();
  // ctx.globalAlpha = 0.1;
  // ctx.fillStyle = 'black';
  ...
  if (obj != null && obj.img != null) {
    ctx.drawImage(
      ...
      foot.position.x, foot.position.y, foot.width, foot.height,
    );
  }
  // ctx.restore();
};

const renderFootHealthBar = (ctx, game, foot): void => {
  ctx.save();
  ctx.translate(
    game.viewPos.x,
    game.viewPos.y,
