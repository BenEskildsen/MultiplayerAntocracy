...
const {tickReducer} = require('./serverTickReducer');
...
const {campaignReducer} = require('./campaignReducer');
const {makeAnt, makeToken, makeSeed} = require('../entities/makeEntity');
...
const rootReducer = (serverState: State, action: Action, clientID, socket, dispatch): State => {
  ...
  switch (action.type) {
    case 'START': {
      const {screen, isExperimental, isCampaign} = action;
      const game = initBaseState(
        {x: 25, y: 25}, 2, [state?.campaign?.species || 'Marauder Ants', 'Leaf Cutter Ants'],
      ...
      return {
        ...
        game,
        campaign: {
          isCampaign, // whether or not we're actually in the campaign
          species: state?.campaign?.species ? state.campaign.species : 'Marauder Ants',
          upgrades: state?.campaign?.upgrades ? state.campaign.upgrades : [],
          currency: 0,

          level: state?.campaign?.level ? state.campaign.level : 0,
          levelName: '',
          levelsCompleted: state?.campaign?.levelsCompleted
            ? state.campaign.levelsCompleted
            : [],
        },
        editor: {
          actions: [],
    ...
    case 'SET_LEVEL': {
      const {numPlayers, gridWidth, gridHeight, actions, upgrades, cpuSpecies} = action.level;
      const game = initBaseState(
        {x: gridWidth, y: gridHeight}, numPlayers,
        [state?.campaign?.species || 'Marauder Ants', ...(cpuSpecies || ['Leaf Cutter Ants'])],
        clientID, state.clients,
      );
      game.session = state;
      game.socketClients = socketClients;
    ...
    case 'SET_PLAYERS_AND_SIZE': {
      ...
      if (action.numPlayers == null && state.game != null) {
        ...
        level.cpuSpecies = [
          state.game.colonies[2]?.species,
          state.game.colonies[3]?.species,
          state.game.colonies[4]?.species,
        ];
      }
      if (action.numPlayers != null || level.reset) {
        const {numPlayers, gridWidth, gridHeight, actions, upgrades, cpuSpecies} = level;
        const game = initBaseState(
          {x: gridWidth, y: gridHeight}, numPlayers,
          [state?.campaign?.species || 'Marauder Ants', ...(cpuSpecies || ['Leaf Cutter Ants'])],
          clientID, state.clients,
    ...
    for (let playerID in game.players) {
      ...
      if (playerQueen != null) {
        ...
        if (game.config[playerID].startWithSeed) {
          const seed1 = makeSeed(game, add(playerQueen.position, {x: 1, y: 0}), 'VINE');
          addEntity(game, seed1);
        }
        if (game.config[playerID].startWithPupaToken) {
          const token = makeToken(
            game,
            add(playerQueen.position, {x: 0, y: 1}),
            playerQueen.playerID,
            'PUPA',
          );
        ...
        for (let i = 0; i < game.config[playerID].startWithDomesticateToken; i++) {
          const token = makeToken(
            game,
            add(playerQueen.position, {x: 1 + i, y: 1 + i}),
            playerQueen.playerID,
            'DOMESTICATE',
          );
          addEntity(game, token);
