...
const {
  ...
  makeForeground,
  makeTermite, makeTermiteEgg, makeDeadTermite,
  makeFoot, makeSpiderWeb,
  makeWorm, makeDeadWorm,
  makeCentipede, makeDeadCentipede,
  makeVine, makeSeed, makeCritterEgg,
  makeCaterpillar,
  makeDeadCaterpillar,
  makeAphid,
  makeDeadAphid,
...
const {getEntityPheromoneSources, getPheromoneAtPosition} = require('../selectors/pheromones');
const {getToken, inTokenInfluence} = require('../selectors/tokens');
...
const gameReducer = (game: Game, action: Action): Game => {
  switch (action.type) {
    case 'UPGRADE': {
      const {path, value, operation, name} = action;
      let obj = game.config;
      ...
      if (name == 'Get A Seed' || name == 'Ant Farm') {
        const seed1 = makeSeed(game, add(queen.position, {x: 1, y: 0}), 'VINE');
      ...
      if (
        name == 'Shorter' || name == 'Thinner' ||
        name == 'Longer' || name == 'Wider'
      ) {
    ...
    case 'ENQUEUE_ENTITY_ACTION': {
      const {entityAction, entity, entityID} = action;
    ...
    case 'SET_EGG_LAYING_COOLDOWN': {
      const {playerID, cooldown} = action;
    ...
    case 'SET_MAKING_CASTE': {
      const {makingCaste, playerID} = action;
    ...
    case 'SET_ABILITY_ACTIVE': {
      const {playerID, active} = action;
      const queen = getQueen(game, playerID);
      // mark dirt
      if (
        (queen.selectedAbility == 'MARK_DIRT' || queen.selectedAbility == 'MARK_CHAMBER'
          || queen.selectedAbility == 'MARK_DIRT_PUTDOWN'
          ) && active == false
        ) {
        queen.digCharge = 1;
    ...
    case 'SET_SELECTED_PHEROMONE': {
      const {playerID, pheromone} = action;
      const queen = getQueen(game, playerID);
    ...
    case 'SET_PHEROMONE_ACTIVE': {
      ...
      // alert/follow/defend
      if (
        (queen.selectedPheromone == 'QUEEN_ALERT' ||
          queen.selectedPheromone == 'QUEEN_PHER' ||
          queen.selectedPheromone == 'PATROL_DEFEND_PHER'
        )
        && active == true && queen.pheromoneActive == false
      ) {
        if (queen.selectedPheromone == 'PATROL_DEFEND_PHER') {
          ...
          getEntityPositions(game, queen)
            .forEach(pos => fillPheromone(game, pos, queen.selectedPheromone, queen.playerID));
        }
      }

      // expand
      if (
        queen.selectedPheromone == 'EXPAND'
        && active == true && queen.pheromoneActive == false
      ) {
      ...
      if (
        queen.selectedPheromone == 'EXPAND'
        && active == false && queen.pheromoneActive == true
      ) {
        game.colonies[playerID].taskNeed['EXPLORE'] -= 20;
      ...
      // contract
      if (
        queen.selectedPheromone == 'CONTRACT'
        && active == true && queen.pheromoneActive == false
      ) {
...
function createEntitiesReducer(game: Game, action): Game {
  ...
  const onlyMakeOne = entityType == 'SPIDER_WEB' ||
    entityType == 'DOODAD';
