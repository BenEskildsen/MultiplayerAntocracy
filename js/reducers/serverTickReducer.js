...
const {
  ...
  floodFillPheromone, reverseFloodFillPheromone,
...
const {takeAcidDamage, removeTrapjaws} = require('../simulation/miscOperations');
const {
  makeAnt, makeEgg, makeLarva, makePupa, makeTermite,
  makeBeetle, makeSpider, makeWorm, makeCentipede,
  makeRolyPoly, makeAphid, makeCaterpillar,
} = require('../entities/makeEntity');
const {render} = require('../render/render');
const {
  getQueen, getPosBehind, getPositionsInFront, onScreen,
  getPositionsBehind,
...
const {
  clamp, closeTo, encodePosition, decodePosition, thetaToDir,
} = require('../utils/helpers');
const {getInterpolatedIndex, getDictIndexStr} = require('../selectors/sprites');
const {
  critterStartCurrentAction, critterDecideAction, antDecideAction,
  termiteDecideAction,
...
const {
  ...
  getNumFoodInTokenRadius, inTokenInfluence,
...
const {updateVines} = require('../simulation/vineOperations');
const {updateSeeds} = require('../simulation/seedOperations');
...
let totalTime = 0;
const tickReducer = (serverState, action: Action, clientID, socket, dispatch): GameState => {
  const {sessions, socketClients, clientToSession} = serverState;
  const state = sessions[clientToSession[clientID]];
...
const doTick = (game: Game): Game => {
  ...
  if (game.time == 1) {
    ...
    //   type: 'INIT', grid: game.grid, config: game.config,
    //   entities: game.entities, TOKEN: game.TOKEN,
    //   gridWidth: game.gridWidth, gridHeight: game.gridHeight,
...
//////////////////////////////////////////////////////////////////////////
const updatePlayerQueen = (game: Game): void => {
  const config = game.config;
  const queen = getQueen(game, game.playerID);
  const ability = queen.selectedAbility;
  if (
    (ability == 'MARK_DIRT' || ability == 'MARK_CHAMBER' || ability == 'MARK_DIRT_PUTDOWN')
    && queen.abilityActive
  ) {
    queen.digCharge = Math.min(
      queen.digCharge + 0.04,
      config[queen.playerID].queenMaxDigCharge,
    );
  }
  const pher = queen.selectedPheromone;
  if (
    queen.pheromoneActive && (
      pher == 'QUEEN_PHER' || pher == 'QUEEN_ALERT' || pher == 'PATROL_DEFEND_PHER'
      || pher == 'QUEEN_DISPERSE'
    )
  ) {
    if (game.time % 75 == 0) {
      getEntityPositions(game, queen)
        .forEach(pos => fillPheromone(game, pos, pher, queen.playerID));
    }
  }
...
const updateAnts = (game: Game): void => {
    ...
    if (
      ant.caste == 'YOUNG_QUEEN' && game.upgradedYoungQueens &&
      ant.task != 'FLY_AWAY' && ant.playerID == game.playerID
    ) {
      antSwitchTask(game, ant, 'FLY_AWAY');
    }
    if (
      ant.caste == 'YOUNG_QUEEN' && ant.task == 'FLY_AWAY' &&
      ant.timeOnTask > randomIn(1500, 2000)
    ) {
...
const updateTermites = (game: Game): void => {
    ...
    if (termite.caste == 'TERMITE_QUEEN' && game.time % 32 == 0) {
      getEntityPositions(game, termite)
        .forEach(pos => fillPheromone(game, pos, 'QUEEN_PHER', termite.playerID));
    }
...
const updateCritters = (game: Game): void => {
  for (const critterType of game.config.critterTypes) {
    for (const id of game[critterType]) {
      ...
      if (critter.type == 'WORM') {
        let ateDirt = false;
        collidesWith(game, critter, ['DIRT'])
          .forEach(dirt => {
            removeEntity(game, dirt);
            ateDirt = true;
          });
        // worm growth
        if (
          ateDirt && Math.random() < game.config.WORM.growthRate
          && critter.segments.length < game.config.WORM.maxSegments
        ) {
      ...
      if (critter.hp <= 0) {
        stackAction(game, critter, makeAction(game, critter, 'DIE', null));
      }
      critter.prevHPAge += 1;
...
//////////////////////////////////////////////////////////////////////////
const updateHose = (game: Game): void => {
  for (const id of game.DOODAD) {
    ...
    if ((game.time + id) % globalConfig.config.hoseDripRate == 0) {
      const waterPos = add(doodad.position, {x: doodad.width, y: doodad.height - 1});
      fillPheromone(
        game, waterPos, 'WATER',
        game.playerID, globalConfig.config.hoseDripAmount,
      );
    }
  }
};

const updateFoot = (game: Game): void => {
...
const updateCPUColonies = (game: Game): void => {
  const config = game.config;
  for (const playerID in game.players) {
    ...
    if (game.time % config[playerID].attackTiming == 0) {
      const antsToAttack = game.ANT
        .map(id => game.entities[id])
        .filter(a => a.playerID == playerID && a.holding == null && a.caste != 'QUEEN');
      antsToAttack.forEach(a => antSwitchTask(game, a, 'ATTACK'));
    }
    // check if the queen just died
    const queen = getQueen(game, playerID);
    if (
      (queen == null || queen.type == 'DEAD_ANT' || queen.type == 'DEAD_TERMITE') &&
      game.colonies[playerID].queenAlive
    ) {
...
const updateRain = (game: Game): void => {
  if (game.rainTicks > 0) {
    game.rainTicks--;
  }
}

const updateRaids = (game: Game): void => {
  if (game.raidTime > 0) {
...
// Update Tokens
//////////////////////////////////////////////////////////////////////////
const updateTokens = (game: Game): void => {
  const config = game.config;
  const queen = getQueen(game, game.playerID);
  ...
  for (const id of game.TOKEN) {
    const token = game.entities[id];
    if (
      token.position == null || token.pheromoneType == 'DOMESTICATE' ||
      token.pheromoneType == 'RAID_PHER'
    ) {
      // game.pheromoneDisplay[token.pheromoneType] = false;
      token.tokenRadius = 1;
      game.pheromoneWorker.postMessage({type: 'SET_TOKEN_RADIUS', token});

      // clean up raid token
      if (token.pheromoneType == 'RAID_PHER' && game.raidTime <= 0) {
        if (queen.holding?.pheromoneType == 'RAID_PHER') {
    ...
    const totalContractedPositions = getPositionsInTokenRadius(
      game, {...token, tokenRadius: token.tokenRadius - 1},
    );
    ...
    if (freePositions.length < 0.25 * totalPositions.length) {
      ...
      freePositions.length - (totalPositions.length - totalContractedPositions.length)
        > 0.25 * totalContractedPositions.length
      && token.tokenRadius > 1
    ) {
      token.tokenRadius -= 1;
      game.pheromoneWorker.postMessage({type: 'SET_TOKEN_RADIUS', token});
...
const updatePatrolTaskNeed = (game: Game): void => {
  for (const playerID in game.players) {
    const queen = getQueen(game, playerID);
...
const updateGoToDirtTaskNeed = (game: Game): void => {
  if (game.time % 500 == 0) {
...
const keepControlledMoving = (game: Game): void => {
  ...
  const queenFollow = getPheromoneAtPosition(
    game, queen.position, 'QUEEN_FOLLOW', queen.playerID
  ) > 0;
  if (queenFollow) {
    moveDir.x = 0;
    moveDir.y = 0;
  ...
  if (
    !equals(moveDir, {x: 0, y: 0}) && !isActionTypeQueued(queen, 'MOVE', true)
    && !isActionTypeQueued(queen, 'MOVE_TURN', true)
    && !isActionTypeQueued(queen, 'TURN') // enables turning in place
    && !isActionTypeQueued(queen, 'DASH')
    && !isActionTypeQueued(queen, 'BURROW_HOME')
  ) {
...
//////////////////////////////////////////////////////////////////////////
const updateCritterLifeCycles = (game): void => {
  // update eggs
  for (const id of game.CRITTER_EGG) {
    ...
    const config = getEntityConfig(game, {...egg, type: egg.critterType});
...
const updateAntLifeCycles = (game): void => {
  ...
  for (const id of game.EGG) {
    ...
    const config = getEntityConfig(game, {...egg, type: 'ANT', caste: egg.caste});
    if (egg.age > config.eggHatchAge && egg.position != null) {
      game.entities[id] = {
  ...
  for (const id of game.PUPA) {
    ...
    const config = getEntityConfig(game, {...pupa, type: 'ANT', caste: pupa.caste});
    if (pupa.age > config.pupaHatchAge && pupa.position != null) {
      ...
      if (pupa.caste == 'SUB_MINIMA') {
        for (let i = 0; i < 3; i++) {
          const subMinima = makeAnt(game, {...pupa.position}, pupa.playerID, 'SUB_MINIMA');
  ...
  for (const id of game.TERMITE_EGG) {
    ...
    const config = getEntityConfig(game, {...egg, type: 'TERMITE', caste: egg.caste});
    if (egg.age > config.eggHatchAge && egg.position != null) {
      game.entities[id] = {
...
const eggLaying = (game: Game, ant: Ant): void => {
  if (
    (ant.caste != 'QUEEN' && !(ant.caste == 'MINIMA' && game.config[ant.playerID].gamergate))
    && ant.caste != 'TERMITE_QUEEN'
  ) return;
  const config = game.config[ant.playerID];

  // player-character queen
  if (ant.playerID == game.playerID && game.controlledEntity?.id == ant.id) {
    ...
    if (ant.eggLayingCharge > 0 && !ant.startEggLaying) {
      ...
      const maxEggCharges = Math.max(
        config.maxEggCharges,
        Math.floor(game.ANT
          .map(id => game.entities[id])
          .filter(a => a.playerID == ant.playerID)
          .length / 10),
      );
      ...
      ant.eggLayingCharge = Math.max(
        config.eggLayingCooldown + ((ant.eggCharges - 1) * 250) -
        getNumFoodInTokenRadius(game, ant.playerID) * 50,
        500,
      );
  ...
  if (
    ant.autopilot ||
      (config.gamergate && ant.caste != 'QUEEN')
  ) {
    if (config.gamergate) {
    ...
    const numLarva = game.LARVA
      .map(id => game.entities[id])
      .filter(l => l.playerID == ant.playerID)
      .length;
    ...
    if (
      ...
      (lessThanMax || ant.id == game.controlledEntity?.id) &&
      inRadius && eggCharges
    ) {
      ...
      if (ant.caste == 'TERMITE_QUEEN') {
        ...
        let numSoldiers = game.TERMITE
          .map(id => game.entities[id])
          .filter(t => t.caste == 'TERMITE_SOLDIER')
          .length;
        numSoldiers += game.TERMITE_EGG
          .map(id => game.entities[id])
          .filter(t => t.caste == 'TERMITE_SOLDIER')
          .length;
...
const updateStaleTiles = (game: Game): void => {
...
const updateCountdown = (game: Game): void => {
  if (game.inCountdown) {
