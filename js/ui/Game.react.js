...
const {initCPUColonySystem} = require('../systems/cpuColonySystem');
const {initTutorialSystem} = require('../systems/tutorialSystem');
const {initRainSystem} = require('../systems/rainSystem');
const {initPheromoneWorkerSystem} = require('../systems/pheromoneWorkerSystem');
const {initUpgradeSystem} = require('../systems/upgradeSystem');
const {
  initKeyboardControlsSystem, clearHotkeys,
} = require('../systems/keyboardControlsSystem');
const ExperimentalSidebar = require('./ExperimentalSidebar.react');
const {
  layEgg, markDirt, bite, dash, markChamber, markDirtPutdown,
...
const {makeToken} = require('../entities/makeEntity');
const {clamp, isMobile, isIpad} = require('../utils/helpers');
...
function Game(props: Props): React.Node {
  ...
  useEffect(() => {
    ...
    const handlers = {
      ...
      leftDown: (state, dispatch, gridPos) =>  {
        ...
        if (queen?.selectedAbility != 'CREATE_RAID') return;
        if (!queen?.abilityActive) return;
  ...
  return (
    ...
    >
      ...
      <Canvas
        dispatch={dispatch}
        tickInterval={tickInterval} queen={queen}
        innerWidth={dims.width}
        innerHeight={dims.height}
        isExperimental={false}
        megaColony={game.config[game.playerID].megaColony}
      />
      {state.campaign.isCampaign  ?
        (<CampaignTopBar
          store={store} dispatch={dispatch} isExperimental={props.isInLevelEditor} />)
        :
      (<TopBar dispatch={dispatch}
        store={store}
        numAnts={numAnts} numKilled={numKilled}
...
function registerHotkeys(dispatch) {
  ...
  dispatch({
    ...
    fn: (s) => {
      ...
      dispatchToServer({type: 'SET_ABILITY_ACTIVE',
        active, playerID: queen.playerID, time: game.time,
      });
  ...
  dispatch({
    ...
    fn: (s) => {
      ...
      if (queen.selectedAbility != 'AUTOPILOT' && queen.selectedAbility != 'CREATE_RAID') {
        ...
        dispatchToServer({type: 'SET_ABILITY_ACTIVE',
          active: false, playerID: queen.playerID, time: game.time,
        });
      }
    }
  });
  dispatch({
    ...
    fn: (s) => {
      ...
      dispatch({type: 'SET_PHEROMONE_ACTIVE',
        active: !queen.pheromoneActive, playerID: queen.playerID,
      });
      dispatchToServer({type: 'SET_PHEROMONE_ACTIVE',
        active: !queen.pheromoneActive, playerID: queen.playerID, time: game.time
      });
    }
  });
  dispatch({
  ...
  dispatch({
    ...
    fn: (s) => {
      ...
      const abilityLength = game.config[queen.playerID]
        .queenAbilities
        .length;
      s.dispatch({type: 'SET_SELECTED_ABILITY', playerID: queen.playerID,
        ability: game.config[queen.playerID]
        .queenAbilities[(abilityIndex + 1) % abilityLength],
      });
      dispatchToServer({type: 'SET_SELECTED_ABILITY', playerID: queen.playerID,
        ability: game.config[queen.playerID]
          .queenAbilities[(abilityIndex + 1) % abilityLength],
        time: game.time,
      });
  ...
  dispatch({
    ...
    fn: (s) => {
      ...
      const pherLength = game.config[queen.playerID]
        .queenPheromones
        .length;
      s.dispatch({type: 'SET_SELECTED_PHEROMONE', playerID: queen.playerID,
        pheromone: game.config[queen.playerID]
          .queenPheromones[(pherIndex + 1) % pherLength],
        });
      dispatchToServer({type: 'SET_SELECTED_PHEROMONE', playerID: queen.playerID,
        time: game.time,
        pheromone: game.config[queen.playerID]
          .queenPheromones[(pherIndex + 1) % pherLength],
        });
  ...
  dispatch({
    ...
    fn: (s) => {
      ...
      const casteLength = game.config[queen.playerID]
        .queenLayingCastes
        .length;
      s.dispatch({type: 'SET_LAYING_CASTE', playerID: queen.playerID,
        caste: game.config[queen.playerID]
          .queenLayingCastes[(casteIndex + 1) % casteLength],
        });
      dispatchToServer({type: 'SET_LAYING_CASTE', playerID: queen.playerID,
        time: game.time,
        caste: game.config[queen.playerID]
          .queenLayingCastes[(casteIndex + 1) % casteLength],
        });
...
function ControlButtons(props) {
  const {dispatch, game, queen, isExperimental} = props;
  ...
  return (
    ...
    >
      ...
      >
      {
        isMobile() ? <Joystick dispatch={dispatch} /> : null
      }
      </div>
    </div>
  );
