...
const {allUpgrades, applyUpgrade} = require('../state/upgrades');
  ...
  store.subscribe(() => {
    ...
    if (trigger.type == 'LARVA' && game.tutorialFlags.larva == null) {
      ...
      if (game.tutorialFlags[flag] == null && !tutorialDismissed && !seenThisCampaign) {
        ...
        dispatch({type: 'SET_TUTORIAL_FLAG', flag});
        dispatch({type: 'SET_MODAL', modal: larvaModal(dispatch, flag)});
        dispatch({
...
function getTutorialTrigger(game, queen) {
  ...
  for (const pos of positions) {
    const tutorialEntities = lookupInGrid(game.grid, pos)
      .map(id => game.entities[id])
      .filter(e => {
        if (e.type == 'LARVA' && e.playerID == queen.playerID) {
          return true;
        }
        if (e.type == 'TOKEN' && (e.pheromoneType == 'DIRT_DROP' ||
          e.pheromoneType == 'COLONY' || e.pheromoneType == 'MOVE_LARVA_PHER'
        )) {
          return true;
        }
        return false;
...
function larvaModal(dispatch, flag) {
  return (
    <SmantModal
      ...
      body={
        'Eggs you lay hatch into larva. Pick up food (the green squares) and feed' +
        ' it to a larva by walking up to it and standing still. A fed larva ' +
        ' will turn into a pupa which will then hatch into a new worker ant.' +
        ' Once your colony is big enough, your worker ants will feed larva themselves.'
      }
      buttons={[
        {label: 'Ok (Enter)', onClick: () => dismissModal(dispatch)},
        {label: 'Don\'t Show This Again', onClick: () => dismissModal(dispatch, flag)},
      ]}
    />
  );
}
...
function foodTokenModal(dispatch, flag) {
  return (
    <SmantModal
      ...
      body={
        ' This token serves as the center of your colony.' +
        ' Your worker ants will gather food (the green squares) around this token.' +
        ' You can pick up this token and place it somewhere else to relocate your colony.' +
        ' Tokens work best in empty areas.'
      }
      buttons={[
        {label: 'Ok (Enter)', onClick: () => dismissModal(dispatch)},
        {label: 'Don\'t Show This Again', onClick: () => dismissModal(dispatch, flag)},
...
function dirtTokenModal(dispatch, game, flag) {
  return (
    <SmantModal
      ...
      body={
        'You\'ve unlocked the ability to emit a pheromone that will mark dirt ' +
        'for your worker ants to dig out! Worker ants that pick up dirt will ' +
        'place it around this token. To mark dirt for digging, use the > button ' +
        'to switch to the "Dig" pheromone, then hold the "Dig" button while facing ' +
        'dirt you want removed. Nearby unoccupied worker ants will go pick up and ' +
        'move the marked dirt to this token.'
      }
      buttons={[
        {label: 'Ok (Enter)', onClick: () => {
          dismissModal(dispatch);
          applyUpgrade(dispatch, game, game.playerID, allUpgrades['Mark Dirt'], true);
        }},
        {label: 'Don\'t Show This Again', onClick: () => {
...
function larvaTokenModal(dispatch, flag) {
  return (
    <SmantModal
      ...
      body={
        'Having your larva spread out all over can make it harder for your worker ' +
        'ants to feed them. Now your worker ants will pick up and move larva to this ' +
        'token to keep larva more organized.'
      }
      buttons={[
        {label: 'Ok (Enter)', onClick: () => dismissModal(dispatch)},
        {label: 'Don\'t Show This Again', onClick: () => dismissModal(dispatch, flag)},
...
module.exports = {initTutorialSystem};
