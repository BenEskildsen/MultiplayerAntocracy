...
const {getQueen, getTotalYoungQueens} = require('../selectors/misc');
const {render} = require('../render/render');
const {getDisplayTime} = require('../utils/helpers');
const {isElectron} = require('../utils/helpers');
const {getNumLevels} = require('../selectors/campaignSelectors');
const {registerSession, postScore} = require('../thunks/clientToServerThunks');
...
const initGameOverSystem = (store) => {
  ...
  store.subscribe(() => {
    ...
    if (
      game.gameOver == 'win' || killedAllGoalCritters
    ) {
    ...
    if (
      game.gameOver == 'lose' ||
      (playerQueen == null || playerQueen.type == 'DEAD_ANT')
    ) {
...
const campaignWon = (store, dispatch, state): void => {
  ...
  dispatch({type: 'SET_MODAL',
    modal: (<SmantModal
      ...
      body={<div>
        Congratulations!
        Your colony has defeated all competition and now rules the land.
        <div>
          {firstWin
            ? "For beating the campaign, you've unlocked an additional species to play as!"
            : null
          }
        </div>
        <Divider />
...
const campaignLevelWon = (store, dispatch, state): void => {
  const game = state.game;
  const map = state.campaign.levelName;
...
const freePlayWinConditions = (store, dispatch, state): void => {
  const game = state.game;
  ...
  const numAnts = game.ANT
    ...
    .length;
  if (
    game.gameOver == 'hit_250' || (
      numAnts >= globalConfig.config.antsToWin &&
      !game.inCountdown &&
      !game.afterCountdown
    )
  ) {
  ...
  if (
    game.gameOver == 'win' ||
    (game.afterCountdown && game.inCountdown)
  ) {
...
const handleCountdownOver = (store, dispatch, state): void => {
  const {game} = state;
...
const RecordScore = (props) => {
  const {
    dispatch, numYoungQueens, totalGameTime, totalAnts,
    map, queenDead, species,
  } = props;
  ...
  let message = "You raised " + totalAnts + " ants and " + numYoungQueens + " Young Queens! " +
    " If you want, you can add a name and register your score for the leaderboards. ";

  if (queenDead) {
    message = "However, you still raised over " + antsToWin + " ants, and " +
      numYoungQueens + " Young Queens. So if you want you can add a name and register " +
      "your score for the leaderboards before you return to the main menu."
  }

  const linkMessage = (
    <div>
      If you liked this game (or have suggestions for improvements),
    ...
    <div>
      ...
      <Button
        ...
        onClick={() => {
          ...
          postScore({username: name, localUser, ...score})
            .then((res) => {
...
const handleGameLoss = (store, dispatch, state, reason): void => {
  ...
  const body = (
    <div>
      {"The queen is dead! Ant colonies can't survive without their queen."}
      {!game.inCountdown ? null :
        <RecordScore
          dispatch={dispatch}
          numYoungQueens={numYoungQueens}
          totalGameTime={game.totalGameTime}
...
const handleGameWon = (store, dispatch, state, reason): void => {
  ...
  const antsToWin = globalConfig.config.antsToWin;
  const message = "You created " + antsToWin + " ants! You now have 10 minutes to go for " +
    " a high score by raising as many young queens as you can."

  const contButton = {
    label: 'Continue to High Score',
    onClick: () => {
      dispatch({type: 'DISMISS_MODAL'});
      dispatch({type: 'SET_HIT_ANTS_TO_WIN'});
      dispatch({type: 'START_TICK'});
    }
  };
  const resetButton = {
