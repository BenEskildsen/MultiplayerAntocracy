...
const layEgg = (dispatch, game, playerID): void => {
  ...
  dispatch({type: 'START_EGG_LAYING', playerID});
  dispatchToServer({type: 'START_EGG_LAYING', playerID, time: game.time});
...
const bite = (dispatch, game): void => {
  ...
  if (
    ...
    (isActionTypeQueued(queen, 'PICKUP') || isActionTypeQueued(queen, 'PUTDOWN'))
  ) {
    return;
  }
...
const markDirt = (dispatch, game, playerID): void => {
  const queen = getQueen(game, playerID);
  ...
  for (let x = 1; x <= magnitude; x++) {
    for (let y = 0; y < queen.width; y++) {
      ...
      markPositions.push({x: xPos, y: yPos});
    }
  }
  ...
  for (let i = 0; i < markPositions.length; i++) {
    ...
    const unoccupied = entityIDs
      .map(id => game.entities[id])
      .filter(e => (
        e.type.slice(0, 4) != 'DEAD' && e.type != 'BACKGROUND' && e.type != 'ANT' &&
        e.type != 'SPIDER_WEB'
      ))
      .length == 0;
  ...
  //   })
  //   .filter(id => id != null);
  const useDirtToken = game.TOKEN
    .map(id => game.entities[id])
    .filter(t => t.pheromoneType == 'DIRT_DROP')
    .length >= 1;
  if (actionType == 'MARK_DIRT' || useDirtToken) {
    dispatch({type: 'MARK_DIRT', dirtIDs, playerID});
    dispatchToServer({type: 'MARK_DIRT', dirtIDs, playerID, time: game.time});
  } else {
    dispatch({type: 'MARK_DIRT_PUTDOWN', playerID, emptyPositions});
    dispatchToServer({type: 'MARK_DIRT_PUTDOWN', playerID, emptyPositions, time: game.time});
  }
};


const markChamber = (dispatch, game, playerID): void => {
  const queen = getQueen(game, playerID);
  const dir = thetaToDir(queen.theta);
  const magnitude = Math.floor(queen.digCharge);
  const markPositions = []
  ...
  const posOffset = [
    {x: 0, y: queen.height - 1},
    {x: -1 * (queen.width - 1), y: 0},
    {x: 0, y: -1 * (queen.height - 1)},
    {x: 0, y: 0},
  ];
  ...
  for (let i = 0; i < markPositions.length; i++) {
    ...
    const unoccupied = entityIDs
      .map(id => game.entities[id])
      .filter(e => e.type.slice(0, 4) != 'DEAD' && e.type != 'BACKGROUND' && e.type != 'ANT')
      .length == 0;
    const dirt = entityIDs.filter(id => game.DIRT.includes(id))[0];
    if (dirt != null) {
      dirtIDs.push(dirt);
...
const markDirtPutdown = (dispatch, game, playerID): void => {
  const queen = getQueen(game, playerID);
  const dir = thetaToDir(queen.theta);
  const magnitude = Math.floor(queen.digCharge);
  let markPositions = []
...
const dash = (dispatch, game): void => {
  ...
  for (let i = 1; i <= dashDist; i++) {
    ...
    const entityAction = makeAction(game, queen, 'DASH', {nextPos}); // TODO: frame offset
    dispatch({type: 'ENQUEUE_ENTITY_ACTION', entity: queen, entityAction});
    // dispatchToServer({type: 'ENQUEUE_ENTITY_ACTION', entity: queen, entityAction, time: game.time});
  }
