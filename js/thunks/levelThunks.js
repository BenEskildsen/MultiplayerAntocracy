...
const {applyUpgrade} = require('../state/upgrades');
const {inferPlayerIDForUpgrade} = require('../selectors/misc');
 ...
 */
function loadLevel(store, levelName, startingUpgrades, synchronous): void {
  ...
  for (const upgrade of startingUpgrades) {
    ...
    applyUpgrade(
      dispatch, game, playerID, upgrade,
      playerID + "" != "1", // levelOnly
    );
  }
