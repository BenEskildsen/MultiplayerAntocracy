...
const {
  getPositionsInFront, getPositionsBehind, getQueen, isFacing,
  getQueenBiteAction, isOnSpiderWeb, getEntityBlockingTypes,
  canDoMove, canLayFood,
} = require('../selectors/misc');
const {
  getPheromoneAtPosition,
} = require('../selectors/pheromones');
const {
 inTokenRadius, inTokenInfluence, getToken,
...
const {
  antLayEgg, antPickupNeighbor, antPickup, antPutdown,
  antDecideMove, antSwitchTask,
} = require('../simulation/antOperations');
const {
  dealDamageToEntity, addTrapjaw, removeTrapjaws,
} = require('../simulation/miscOperations');
const {
  makeFood, makeTermiteEgg, makeCritterEgg,
} = require('../entities/makeEntity');
const {footFall, footStomp, footLift} = require('../simulation/footOperations');
...
const critterStartCurrentAction = (
  ...
  switch (curAction.type) {
    case 'PICKUP': {
      if (curAction.payload != null) {
        const {pickupID, position} = curAction.payload;
    ...
    case 'PUTDOWN':
      antPutdown(game, entity, curAction?.payload?.position);
      break;
    case 'FEED':
...
const burrowHome = (game: Game, ant: Ant, pheromoneType: string): void => {
 ...
 */
const entityDoMove = (game: Game, entity: Entity, nextPos: Vector): boolean => {
  ...
  if (
    entity.caste == 'QUEEN' &&
    curAction == 'DASH' &&
    game.config[entity.playerID].queenArmored
  ) {
    // HACK: deal the damage along the path here
    const collisions = collidesWith(game, entity,
      [
        ...globalConfig.config.critterTypes,
        'EGG', 'CRITTER_EGG', 'LARVA', 'PUPA', 'TERMITE_EGG',
        'FOOD', 'ANT', 'TERMITE',
      ]);
  ...
  if (
    entity.type == 'ANT' && entity.holding != null && entity.holding.type == 'FOOD' &&
    getPheromoneAtPosition(game, entity.position, 'COLONY', entity.playerID) == 0
  ) {
  ...
  if (
    isOnSpiderWeb(game, entity) && entity.type != 'SPIDER' &&
    Math.random() < game.config.spiderWebStuckProbability
  ) {
    entity.stuck = true;
  }
...
const entityDie = (game: Game, entity: Entity): void => {
  ...
  if (entity.caste == 'MINIMA' && game.config[entity.playerID].acidDeath) {
    getEntityPositions(game, entity)
      .forEach(pos => fillPheromone(game, pos, 'ACID', entity.playerID));
  }

  // turn into food:
  if (
    game.config.critterTypes.includes(entity.type) || entity.type == 'TERMITE'
  ...
  if (entity.type == 'SPIDER') {
    const web = lookupInGrid(game.grid, entity.position)
      .map(id => game.entities[id])
      .filter(e => e.type == 'SPIDER_WEB')[0];
    if (web != null) {
  ...
  if (entity.task != null) {
    if (
      entity.task == 'EXPLORE' || entity.task == 'RETRIEVE' ||
      entity.task == 'GO_TO_DIRT' || entity.task == 'FEED_LARVA'
    ) {
...
const termiteLayEgg = (game: Game, termite: Termite, layingCaste: TermiteCaste): Game => {
...
const critterLayEgg = (game: Game, critter: Entity): Game => {
  const config = game.config;
  const positions = getPositionsBehind(game, critter);
  for (const eggPosition of positions) {
    const occupied = lookupInGrid(game.grid, eggPosition)
      .map(id => game.entities[id])
      .filter(e => e.id != critter.id && e.type.slice(0, 4) != 'DEAD' && e.type != 'BACKGROUND')
      .length > 0;
    const inGrid = insideGrid(game.grid, eggPosition);
...
const critterDecideAction = (game: Game, entity: Entity): void => {
  ...
  if (
    entity.eggCharges > 0 &&
    entity.age > (maxEggs - entity.eggCharges + 1) * cooldown &&
    (entity.type != 'WORM' ||  entity.segments.length >= maxSegments)
  ) {
  ...
  let freeNeighbors = getFreeNeighborPositions(
    game, entity, getEntityBlockingTypes(game, entity),
  )
...
const antFeedNeighbor = (game: Game, ant: Ant): Game => {
  ...
  const facingLarva = neighboringLarva
    .filter(l => isFacing(ant, l.position));
...
const termiteDecideAction = (game: Game, termite: Termite): void => {
  if (termite.caste == 'TERMITE_QUEEN') return;

  const config = game.config[termite.playerID];
  if (termite.holding == null) {
    // FIGHT
    const targets = getNeighborEntities(game, termite, true)
      .filter(e => {
        return (
          game.config.critterTypes.includes(e.type) ||
          (e.type == 'ANT' && e.playerID != termite.playerID) ||
          (
            (e.type == 'LARVA' || e.type == 'EGG' || e.type == 'PUPA') &&
            e.playerID != termite.playerID) ||
          (e.type == 'TERMITE' && e.playerID != termite.playerID)
        );
      });
    if (targets.length > 0) {
      const target = oneOf(targets);
      let actionType = 'BITE';
      if (
        (target.caste == 'MINIMA' && termite.caste == 'TERMITE_WORKER') ||
        (target.caste == 'TERMITE_WORKER' && termite.caste == 'TERMITE_WORKER')
      ) {
        actionType = 'GRAPPLE';
      }
  ...
  for (const putdownPos of possiblePutdownPositions) {
    const putdownLoc = {position: putdownPos, playerID: termite.playerID};
    ...
    // if holding egg and near putdown token, put it down
    if (
      (holdingEgg || termite.task == 'MOVE_EGG')
      && inTokenRadius(game, putdownLoc, 'EGG')
      && !occupied
    ) {
