...
const {add, subtract, magnitude} = require('../utils/vectors');
const {
  makeAction, isActionTypeQueued,
  queueAction, stackAction, cancelAction,
} = require('../simulation/actionQueue');
const {removeEntity, addEntity} = require('../simulation/entityOperations');
    ...
    if (!entity.actions) {
      ...
      if (entity.type == 'VINE') {
        ...
        if (Math.random() < config.VINE.seedRate && entity.subType == 'STALK') {
          ...
          entity.subType == 'STALK' &&
          (Math.random() < config.VINE.foodRateOnDeath || !game.vineDied)
        ) {
...
const takeAcidDamage = (game: Game, entity: Entity): void => {
  let totalAcid = 0;
...
const addTrapjaw = (game: Game, entity: Entity, trapjawAnt: Ant): void => {
...
const removeTrapjaws = (game: Game, entity: Entity): void => {
  if (entity?.trapjaws == null || entity.trapjaws.length == 0) return;
