...
const {makeVine, makeFood, makeSeed} = require('../entities/makeEntity');
const {addEntity, removeEntity} = require('../simulation/entityOperations');
const {add, equals, subtract, makeVector, vectorTheta} = require('../utils/vectors');
const {
...
const {getPheromoneAtPosition, getPheromoneBlockers} = require('../selectors/pheromones');
const {getEntityConfig} = require('../selectors/config');

function updateVines(game: Game) {
  ...
  for (const vineID of game.VINE) {
    ...
    const nearDomesticate = getFreeNeighborPositions(
      game, vine, config[game.playerID].pheromoneBlockingTypes,
    ).filter(p => {
      return getPheromoneAtPosition(game, p, 'DOMESTICATE', game.playerID) > 0 &&
        config[game.playerID].growthPheromone;
    })
    .length > 0;
    if (nearDomesticate) {
...
function growBud(game, vine) {
  const config = game.config;
  vine.forked = false;
  const candidatePositions = getFreeNeighborPositions(
    game, vine, getEntityConfig(game, vine).blockingTypes,
  ).filter(pos => !isDiagonalMove(vine.position, pos));
  // "secure" positions neighbor dirt or stone
  const securePositions = candidatePositions.filter(pos => {
    const neighborEntities = getNeighborEntities(game, {...vine, position: pos});
    const isSecure = neighborEntities
...
function shouldGrow(game, vine) {
  ...
  let rate = vine.forked
    ? config['VINE'].forkedGrowthRate
    : config['VINE'].growthRate;
