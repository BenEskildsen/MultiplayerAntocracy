...
const getQueen = (game: Game, playerID: PlayerID): ?ant => {
...
const getEntityBlockingTypes = (game: Game, entity: Entity): Array<EntityType> => {
  ...
  if (
    entity.caste == 'QUEEN' || entity.caste == 'MEDIA' || entity.caste == 'MAJOR'
  ...
  if (entity.id == game.controlledEntity?.id && token) {
    blockers = [];
  }
  ...
  const isBlocked = lookupInGrid(game.grid, entity.position)
    .map(id => game.entities[id])
    .filter(e => e.id != entity.id)
    .filter(e =>
      blockers.includes(e.type) && (e.type != 'ANT' || e.caste == 'MAJOR' || e.caste == 'MEDIA')
    )
    .length > 0;
 ...
 */
const getQueenBiteAction = (game: Game, queen: Ant): EntityAction => {
  if (queen.type == 'DEAD_ANT') {
    return {type: 'BITE', target: null, duration: 1};
  }
  ...
  for (const pos of positionsInFront) {
    const maybeTarget = lookupInGrid(game.grid, pos)
      .map(id => game.entities[id])
      .filter(e => {
        return (
          config.critterTypes.includes(e.type) ||
          (e.type == 'ANT' && e.playerID != queen.playerID) ||
          (e.type == 'TERMITE' && e.playerID != queen.playerID) ||
          (e.type == 'VINE')
  ...
  for (const pickupPos of positionsInFront) {
    const pickup = lookupInGrid(game.grid, pickupPos)
      .map(id => game.entities[id])
      .filter(e => config[queen.playerID].queenPickupTypes.includes(e.type))
      [0];
    if (pickup != null) {
      return makeAction(game, queen, 'PICKUP', {pickupID: pickup.id, position: pickupPos});
    }
  }
 ...
 */
const getOtherPlayerID = (game: Game, playerID: PlayerID): PlayerID => {
...
const isOnSpiderWeb = (game: Game, entity: Entity): boolean => {
  return collidesWith(game, entity, ['SPIDER_WEB']).length > 0;
}

const isDead = (entity: Entity): boolean => {
...
const isDirtBlockage = (game: Game, ant: Ant, dirt: Dirt): boolean => {
  const positions = getNeighborPositions(game, ant, true);
  let maxPassThrough = 0;
  let maxPassPos = {x: 0, y: 0};
  for (const pos of positions) {
    const passThrough = getPheromoneAtPosition(
      game, pos, 'PASS_THROUGH_COLONY', game.playerID,
    );
    // if the space is occupied by stone then we can't pass through there
    const isStone = lookupInGrid(game.grid, pos)
      .map(id => game.entities[id])
      .filter(e => e.type == 'STONE' || e.type == 'DOODAD')
      .length > 0;

    const isDirt = lookupInGrid(game.grid, pos)
      .map(id => game.entities[id])
    ...
    if (isStone) continue;
    if (
      (isDirt && passThrough > maxPassThrough) ||
      (!isDirt && passThrough >= maxPassThrough)
    ) {
...
const canLayFood = (game: Game, ant: Ant): boolean => {
...
const getTotalYoungQueens = (game, includeDead) => {
  let deadYoungQueens = game.DEAD_ANT
    .map(id => game.entities[id])
    .filter(a => {
