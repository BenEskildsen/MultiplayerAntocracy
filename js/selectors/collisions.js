  ...
  const collisions = lookupInGrid(game.grid, pos)
    ...
    .filter(e => {
      ...
      if (e.type == 'ANT' && entity.type == 'ANT' && blockingTypes.includes('ANT')) {
        ...
        if (
          entity.caste == 'QUEEN' && game.config[entity.playerID].queenArmored
          && curAction == 'DASH' &&
          !blockingTypes.includes('ANT') // HACK this is required for dashing through
        ) {
          return false;
        }
        ...
        if (
          (entity.caste != e.caste || entity.caste == 'QUEEN')
          && e.playerID == entity.playerID
        ) {
          return false;
        } else {
          return true;
