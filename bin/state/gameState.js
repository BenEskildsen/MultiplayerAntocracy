...
const initPlayerAndColony = (type, id, name, species) => {
  return {
    player: {
      id,
      name,
...
const initBaseState = (gridSize, numPlayers, speciesArray, playerID, clients) => {
  ...
  const game = {
    ...
    queens: {},
    // map of playerID to queen entityID
    totalYoungQueens: 0,
    // young queens that flew away

    upgradedAt: [],
    // track upgrade thresholds you've already reached so that
  ...
  for (let i = 1; i <= clients.length; i++) {
    ...
    const playerAndColony = initPlayerAndColony('HUMAN', i, 'You', speciesArray[i - 1] || 'Marauder Ants');
  ...
  for (let i = 2; i < numPlayers; i++) {
    const {
      player,
      colony
    } = initPlayerAndColony('COMPUTER', i + 1, 'Enemy', speciesArray[i] || 'Leaf Cutter Ants');
