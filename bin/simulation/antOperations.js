...
const antPickup = (game, ant, entity, pickupPos) => {
  ...
  if (ant.caste == 'QUEEN' && !config.queenPickupTypes.includes(entity.type)) return game;
  ...
  if (entity.type == 'LARVA' && game.config[entity.playerID].spikedLarva != null && entity.playerID != ant.playerID) {
    dealDamageToEntity(game, ant, game.config[entity.playerID].spikedLarva);
  }

  // switch enemy egg/larva/pupa to ant's side
  if (entity.playerID != null && entity.playerID != ant.playerID && entity.species == ant.species && entity.type != 'TOKEN') {
    entity.playerID = ant.playerID;
  }

  // update task need if ant not doing go_to_dirt picks up marked dirt
  if (ant.task != 'GO_TO_DIRT' && entity.type == 'DIRT' && entity.marked == ant.playerID && game.colonies[ant.playerID].taskNeed['GO_TO_DIRT'] > 0) {
  ...
  if (entity.type == 'FOOD') {
    ...
    let distToColony = game.config[ant.playerID].COLONY.quantity - getPheromoneAtPosition(game, ant.position, 'COLONY', ant.playerID);
    ...
    for (let playerID in game.players) {
      ...
      if (colony.foodMarkedForRetrieval[entity.id]) {
        if (ant.task != 'RETRIEVE' && playerID == ant.playerID || ant.playerID != playerID) {
    ...
    getNeighborEntities(game, entity).filter(e => e.type == 'FOOD').forEach(f => {
      ant.foodPherQuantity = distToColony * 2.1;
      if (!colony.foodMarkedForRetrieval[f.id] && !inTokenRadius(game, {
        ...f,
    ...
    getFreeNeighborPositions(game, entity, getPheromoneBlockers('FOOD')).forEach(pos => fillPheromone(game, pos, 'FOOD', ant.playerID, ant.foodPherQuantity));
  }
...
const antPickupNeighbor = (game, ant) => {
  const config = game.config[ant.playerID];
  ...
  if (ant.caste == 'MEDIA') {
    // && game.colonies[ant.playerID].species == 'Leaf Cutter Ants') {
    pickupTypes.push('VINE');
  }
  const neighbors = getNeighborEntitiesAndPosition(game, ant, true).filter(n => pickupTypes.includes(n.entity.type));
      ...
      case 'FOOD':
        {
          ...
          if (!inTokenRadius(game, {
            ...food,
            playerID: ant.playerID
          }, 'COLONY') || game.colonies[ant.playerID].taskNeed['FEED_LARVA'] > 0) {
            stackAction(game, ant, makeAction(game, ant, 'PICKUP', {
              pickupID: food.id,
              position
            }));
            numStacked++;
      ...
      case 'EGG':
        {
          ...
          if (inTokenRadius(game, {
            ...egg,
            playerID: ant.playerID
          }, 'EGG') || !tokenExists(game, ant.playerID, 'EGG')) break;
          stackAction(game, ant, makeAction(game, ant, 'PICKUP', {
            pickupID: egg.id,
            position
          }));
      ...
      case 'LARVA':
        {
          ...
          if (inTokenRadius(game, {
            ...larva,
            playerID: ant.playerID
          }, 'MOVE_LARVA_PHER') || !tokenExists(game, ant.playerID, 'MOVE_LARVA_PHER') || ant.task == 'RETRIEVE') break;
          stackAction(game, ant, makeAction(game, ant, 'PICKUP', {
            pickupID: larva.id,
            position
          }));
      ...
      case 'PUPA':
        {
          ...
          if (inTokenRadius(game, {
            ...pupa,
            playerID: ant.playerID
          }, 'PUPA') || !tokenExists(game, ant.playerID, 'PUPA')) break;
          stackAction(game, ant, makeAction(game, ant, 'PICKUP', {
            pickupID: pupa.id,
            position
          }));
...
const antPutdown = (game, ant, position) => {
  if (ant.holding == null) {
    if (ant.task != 'DEFEND') {
  ...
  for (const putDownPos of putDownPositions) {
    const blockers = lookupInGrid(game.grid, putDownPos).map(id => game.entities[id]).filter(e => e.type.slice(0, 4) != 'DEAD' && e.type != 'BACKGROUND' && e.type != 'SPIDER_WEB');
    ...
    if (ant.holding.type != 'VINE') {
      ...
      for (let x = 0; x < leaf.width; x++) {
        for (let y = 0; y < leaf.height; y++) {
          addEntity(game, makeFood(game, add(putDownPos, {
            x,
            y
          })));
        }
    ...
    if (ant.holding.type != 'FOOD' && ant.holding.type != 'VINE') {
      // ie DIRT, EGG, LARVA, PUPA,
      antSwitchTask(game, ant, 'RETURN');
    } else {
...
const antSwitchTask = (game, ant, task) => {
  ...
  if (ant.caste == 'YOUNG_QUEEN' && task != 'DEFEND' && task != 'WANDER' && task != 'FLY_AWAY' && !game.config[ant.playerID].youngQueensDoTasks) {
  ...
  if (ant.task == 'RETRIEVE' && (task != 'RETURN' || ant.holding == null || ant.holding.type != 'FOOD')) {
 ...
 */
const antLayEgg = (game, ant, layingCaste) => {
  var _ant$actions$;
  const config = game.config[ant.playerID];
  ...
  for (const eggPosition of positions) {
    const occupied = lookupInGrid(game.grid, eggPosition).filter(id => ant.id != id && blockers.includes(game.entities[id].type)).length > 0;
  ...
  if (((_ant$actions$ = ant.actions[0]) === null || _ant$actions$ === void 0 ? void 0 : _ant$actions$.type) == 'LAY') {
...
const antDecideMove = (game, ant) => {
  var _game$controlledEntit, _game$controlledEntit2;
  if (ant.id == ((_game$controlledEntit = game.controlledEntity) === null || _game$controlledEntit === void 0 ? void 0 : _game$controlledEntit.id)) {
  ...
  const freeNeighbors = getFreeNeighborPositions(game, ant, blockers).filter(pos => canDoMove(game, ant, pos).result);
  ...
  if (ant.caste == 'YOUNG_QUEEN' && !config.youngQueensDoTasks && ant.task != 'FLY_AWAY' || ant.caste == 'QUEEN' && ant.id != ((_game$controlledEntit2 = game.controlledEntity) === null || _game$controlledEntit2 === void 0 ? void 0 : _game$controlledEntit2.id)) {
    taskConfig = {};
  ...
  if (getPheromoneAtPosition(game, ant.position, 'QUEEN_DISPERSE', game.playerID) > 0) {
    playerID = game.playerID;
  }
  const basePher = getPheromonesInCell(game.grid, ant.position, playerID);
  const pheromoneNeighbors = freeNeighbors.map(pos => getPheromonesInCell(game.grid, pos, playerID));
  ...
  for (let i = 0; i < freeNeighbors.length; i++) {
    ...
    neighborScores[i] += (pher.ALERT - basePher.ALERT) * taskConfig.ALERT;
    neighborScores[i] += (pher.QUEEN_ALERT - basePher.QUEEN_ALERT) * taskConfig.QUEEN_ALERT;

    // enemy ants avoid your disperse
    const avoidDisperse = getPheromoneAtPosition(game, ant.position, 'QUEEN_DISPERSE', game.playerID) > 0 && ant.playerID != game.playerID ? -1 : 1;
    neighborScores[i] += (pher.QUEEN_DISPERSE - basePher.QUEEN_DISPERSE) * taskConfig.QUEEN_DISPERSE * avoidDisperse;
    ...
    neighborScores[i] += (pher.QUEEN_PHER - basePher.QUEEN_PHER) * taskConfig.QUEEN_PHER;
    neighborScores[i] += (pher.QUEEN_FOLLOW - basePher.QUEEN_FOLLOW) * taskConfig.QUEEN_FOLLOW;
    neighborScores[i] += (pher.CRITTER_PHER - basePher.CRITTER_PHER) * taskConfig.CRITTER_PHER;
    if (ant.task == 'EXPLORE') {
      // TODO: don't hardcode explore values
      neighborScores[i] += (400 - Math.abs(pher.COLONY - 300) - (400 - Math.abs(basePher.COLONY - 300))) * taskConfig.COLONY;
      ...
      if (otherPlayerID != null) {
        ...
        const otherPheromoneNeighbors = freeNeighbors.map(pos => getPheromonesInCell(game.grid, pos, otherPlayerID));
        const otherPher = otherPheromoneNeighbors[i];
        otherColonyAttractorScore = (otherPher.COLONY - otherBasePher.COLONY) * (taskConfig.COLONY / 10);
      }
      ...
      const patrolRadiusScore = (pher.COLONY - basePher.COLONY) * baseDist * taskConfig.COLONY;
      ...
      } else if (patrolRadiusScore < 0 && otherColonyAttractorScore > 0 && game.config[playerID].COLONY.quantity - basePher.COLONY < radius) {
        neighborScores[i] += otherColonyAttractorScore;
      } else {
        neighborScores[i] += patrolRadiusScore + otherColonyAttractorScore;
      }
    } else {
      neighborScores[i] += (pher.COLONY - basePher.COLONY) * taskConfig.COLONY;
    }
    neighborScores[i] += (pher.PATROL_DEFEND_PHER - basePher.PATROL_DEFEND_PHER) * taskConfig.PATROL_DEFEND_PHER;
    neighborScores[i] += (pher.EGG - basePher.EGG) * taskConfig.EGG;
    neighborScores[i] += (pher.MOVE_LARVA_PHER - basePher.MOVE_LARVA_PHER) * taskConfig.MOVE_LARVA_PHER;
    neighborScores[i] += (pher.LARVA - basePher.LARVA) * taskConfig.LARVA;
    neighborScores[i] += (pher.PUPA - basePher.PUPA) * taskConfig.PUPA;
    neighborScores[i] += (pher.PASS_THROUGH_COLONY - basePher.PASS_THROUGH_COLONY) * taskConfig.PASS_THROUGH_COLONY;
    neighborScores[i] += (pher.RAID_PHER - basePher.RAID_PHER) * taskConfig.RAID_PHER;
    neighborScores[i] += (pher.DIRT_DROP - basePher.DIRT_DROP) * taskConfig.DIRT_DROP;
    neighborScores[i] += (pher.MARKED_DIRT_PHER - basePher.MARKED_DIRT_PHER) * taskConfig.MARKED_DIRT_PHER;
  ...
  queueAction(game, ant, makeAction(game, ant, 'MOVE', {
    nextPos
  }));
  return game;
};
const antDecideTask = (game, ant, nextPos) => {
  const config = game.config[ant.playerID];
  // queens only wander
  ...
  if (ant.caste == 'YOUNG_QUEEN' && ant.task != 'WANDER' && ant.task != 'DEFEND' && !config.youngQueensDoTasks) {
    antSwitchTask(game, ant, 'WANDER');
    return;
  }
  ...
  if (pherAtCell.QUEEN_ALERT > 0 && ant.task != 'DEFEND' && ant.task != 'PATROL_DEFEND' && pherAtCell.PATROL_DEFEND_PHER == 0) {
    antPutdown(game, ant);
    antSwitchTask(game, ant, 'DEFEND');
    return;
  }
  // don't drop what you're doing for regular alert
  if (pherAtCell.ALERT && ant.holding == null && ant.task != 'DEFEND' && ant.task != 'PATROL_DEFEND' && pherAtCell.PATROL_DEFEND_PHER == 0) {
    antSwitchTask(game, ant, 'DEFEND');
    return;
  }

  // stay on DEFEND if time on task < 70 milliseconds
  if (ant.task == 'DEFEND' && (ant.timeOnTask < 70 || pherAtCell.ALERT > 0 || pherAtCell.QUEEN_ALERT > 0) && pherAtCell.PATROL_DEFEND_PHER == 0) {
    antPutdown(game, ant);
    return;
  }

  // switch back to wandering if defending without pheromone
  if (ant.task == 'DEFEND' && ant.timeOnTask > 70 && pherAtCell.ALERT == 0 && pherAtCell.QUEEN_ALERT == 0) {
    antSwitchTask(game, ant, 'WANDER');
    return;
  }
  if (ant.task == 'PATROL_DEFEND' && (pherAtCell.PATROL_DEFEND_PHER == 0 || ant.holding != null)) {
    antSwitchTask(game, ant, 'WANDER');
    return;
  }
  ...
  if (pherAtCell.FOOD > 0 && ant.holding == null && (ant.task == 'WANDER' || ant.task == 'PATROL' || ant.task == 'EXPLORE' || ant.task == 'RETURN') && game.colonies[ant.playerID].taskNeed['RETRIEVE'] > 0) {
    antSwitchTask(game, ant, 'RETRIEVE');
    return;
  }

  // go to dirt if on pheromone cell
  if (pherAtCell.MARKED_DIRT_PHER > 0 && ant.holding == null && (ant.task == 'WANDER' || ant.task == 'PATROL' || ant.task == 'EXPLORE' || ant.task == 'RETURN') && game.colonies[ant.playerID].taskNeed['GO_TO_DIRT'] > 0) {
    antSwitchTask(game, ant, 'GO_TO_DIRT');
    return;
  }

  // if Returning but holding nothing, see if near-ish to token to switch to WANDER
  if (ant.task == 'RETURN' && ant.holding == null && inTokenInfluence(game, ant, 'COLONY')) {
    antSwitchTask(game, ant, 'WANDER');
    return;
  }
  ...
  if (holdingFood && ant.task != 'RETURN' && ant.task != 'FEED_LARVA') {
    const position = ant.holding.prevPosition != null ? ant.holding.prevPosition : ant.position;
    if (inTokenRadius(game, {
      ...ant,
      position
    }, 'COLONY') && game.colonies[ant.playerID].taskNeed['FEED_LARVA'] > 0) {
  ...
  if (holdingFood) {
    const position = ant.holding.prevPosition != null ? ant.holding.prevPosition : ant.position;
    if (ant.task == 'RETURN' && pherAtCell.COLONY == game.config[ant.playerID].COLONY.quantity && game.colonies[ant.playerID].taskNeed['FEED_LARVA'] > 0) {
      antSwitchTask(game, ant, 'FEED_LARVA');
    } else if (ant.task != 'RETURN' && ant.task == 'FEED_LARVA' && game.colonies[ant.playerID].taskNeed['FEED_LARVA'] < 0
  ...
  if (pherAtCell.FOOD == 0 && !holdingFood && ant.task == 'RETRIEVE') {
    if (Math.random() < 0.5 && game.colonies[ant.playerID].taskNeed['EXPLORE'] > 0) {
  ...
  if (pherAtCell.MARKED_DIRT_PHER == 0 && !holdingDirt && ant.task == 'GO_TO_DIRT') {
    antSwitchTask(game, ant, 'RETURN');
    return;
  }

  // if Returning but holding nothing, switch to Wander if in QUEEN_PHER
  if (ant.holding == null && ant.task == 'RETURN' && getPheromonesInCell(game.grid, ant.position, ant.playerID).QUEEN_PHER > 0) {
    antSwitchTask(game, ant, 'WANDER');
    return;
  }
  ...
  if (ant.task == 'WANDER' && ant.playerID != game.playerID && getPheromoneAtPosition(game, ant.position, 'COLONY', game.playerID) == 0 && Math.random() < 0.01) {
    antSwitchTask(game, ant, 'GO_TO_BLOCKAGE');
    return;
  }

  // switch back out of this task if there's no more blockage
  if (ant.task == 'GO_TO_BLOCKAGE' && getPheromoneAtPosition(game, ant.position, 'COLONY', game.playerID) > 0) {
    antSwitchTask(game, ant, 'WANDER');
    return;
  }

  // if Wandering, switch to Exploring with certain probability
  if (ant.task == 'WANDER' && Math.random() < config.exploreRate && !holdingFood && !holdingDirt && game.colonies[ant.playerID].taskNeed['EXPLORE'] > 0) {
    antSwitchTask(game, ant, 'EXPLORE');
    return;
  }

  // if Wandering, switch to Patrolling with certain probability
  if (ant.task == 'WANDER' && Math.random() < 0.1 && ant.holding == null && game.colonies[ant.playerID].taskNeed['PATROL'] > 0) {
    antSwitchTask(game, ant, 'PATROL');
    return;
  }

  // if Exploring and near the queen, switch to wander
  if (ant.task == 'EXPLORE' && getPheromonesInCell(game.grid, ant.position, ant.playerID).QUEEN_PHER > 0) {
    antSwitchTask(game, ant, 'WANDER');
  }

  // if Exploring or Patrolling, but with negative task need, switch to return
  if (ant.task == 'EXPLORE' && game.colonies[ant.playerID].taskNeed['EXPLORE'] < 0 || ant.task == 'PATROL' && game.colonies[ant.playerID].taskNeed['PATROL'] < 0) {
    antSwitchTask(game, ant, 'RETURN');
    return;
  }
  ...
  if (pherAtCell.PATROL_DEFEND_PHER > 0 && ant.holding == null) {
...
const termiteDecideTask = (game, termite, nextPos) => {
