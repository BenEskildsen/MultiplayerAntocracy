...
const footFall = (game, foot) => {
  foot.state = 'falling';
  const config = game.config;
  changeEntitySize(game, foot, game.config.FOOT.width, game.config.FOOT.height);
};
const footStomp = (game, foot) => {
  foot.state = 'stomping';
  foot.numStomps += 1;
  const config = game.config;
  ...
  const squishables = ['ANT', 'EGG', 'LARVA', 'PUPA', 'TERMITE', 'TERMITE_EGG', ...config.critterTypes];
  const toSquish = collidesWith(game, foot, squishables);
  ...
  const toCrater = collidesWith(game, foot, dirtTypes);
  for (const dirt of toCrater) {
    if (Math.random() < 0.5) {
      removeEntity(game, dirt);
    } else {
      const freePositions = getFreeNeighborPositions(game, foot, [...squishables, ...dirtTypes, 'TOKEN', 'BACKGROUND'], true);
      if (freePositions.length == 0) {
        const nextRingPositions = getFreeNeighborPositions(game, {
          ...foot,
          position: subtract(foot.position, {
            x: 1,
            y: 1
          }),
          width: foot.width + 2,
          height: foot.height + 2
        }, [...squishables, ...dirtTypes, 'TOKEN', 'BACKGROUND'], true);
...
const footLift = (game, foot) => {
  ...
  let nextPos = {
    ...game.entities[oneOf(game.BACKGROUND)].position
  };
  queueAction(game, foot, makeAction(game, foot, 'MOVE', {
    nextPos
  }));
};
module.exports = {
