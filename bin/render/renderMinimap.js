...
const renderMinimap = (ctx, game, dims) => {
  ...
  ctx.fillRect(x, y, Math.min(game.gridWidth - x, viewWidth + 1), Math.min(game.gridHeight - y, viewHeight + viewPos.y, viewHeight + 1));
...
const renderEntity = (ctx, game, dims, id, renderFn) => {
...
const renderTile = (ctx, game, dims, entity) => {
  ...
  if (entity.position.x + entity.width > dims.viewWidth + dims.viewPos.x) {
    width = entity.width - Math.abs(entity.position.x + entity.width - (dims.viewWidth + dims.viewPos.x));
  }
  let height = entity.height;
  if (entity.position.y + entity.height > dims.viewHeight + dims.viewPos.y) {
    height = entity.height - Math.abs(entity.position.y + entity.height - (dims.viewHeight + dims.viewPos.y));
  }
...
const renderPill = (ctx, game, dims, entity) => {
...
const onMinimap = (dims, entity) => {
  ...
  return x >= viewPos.x - 1 && y >= viewPos.y - 1 && x + width <= viewWidth + viewPos.x + 1 && y + height <= viewHeight + viewPos.y + 1;
};
module.exports = {
  renderMinimap
};