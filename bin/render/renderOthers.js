...
const renderFood = (ctx, game, food) => {
...
const renderDirt = (ctx, game, dirt) => {
...
const renderStone = (ctx, game, stone) => {
...
const renderDoodad = (ctx, game, doodad) => {
  ...
  if (sprite != null) {
    ctx.drawImage(sprite, doodad.position.x, doodad.position.y, width, height);
  }
  if (doodad.sprite == 'QUESTION') {
    ctx.translate(-0.5, -0.5);
  }
};
const renderSpiderWeb = (ctx, game, spiderWeb) => {
  const sprite = game.sprites.SPIDER_WEB;
  if (sprite != null) {
    ctx.drawImage(sprite, spiderWeb.position.x, spiderWeb.position.y, spiderWeb.width, spiderWeb.height);
  }
};
const renderBackground = (ctx, game, tile) => {
...
const renderForeground = (ctx, game, tile) => {
  const obj = getForegroundSprite(game, tile);
  if (obj == null || obj.img == null) return;
  ctx.drawImage(obj.img, obj.x, obj.y, obj.width, obj.height, tile.position.x, tile.position.y, game.gridWidth, game.gridHeight);
};
const renderTokenSymbol = (ctx, game, entity) => {
  const sprite = game.sprites[entity.type];
  if (sprite != null) {
    ctx.drawImage(sprite, entity.position.x, entity.position.y, entity.width, entity.height);
  }
};
const renderEgg = (ctx, game, entity) => {
  const egg = entity;
  ...
  for (let x = 0; x <= num; x++) {
    for (let y = 0; y <= num; y++) {
      if (timeToHatch < 120) {
        ...
        if (sprite != null) {
          ctx.drawImage(sprite, 32 * Math.floor((119 - timeToHatch) / 20), 0, 32, 32, position.x + x / 2, position.y + y / 2, egg.width * scale, egg.height * scale);
        }
      } else {
        const sprite = game.sprites.EGG;
        if (sprite != null) {
          ctx.drawImage(sprite, position.x + x / 2, position.y + y / 2, egg.width * scale, egg.height * scale);
        }
      }
    }
  }
};
const renderCritterEgg = (ctx, game, entity) => {
  const egg = entity;
  const sprite = game.sprites.EGG;
  if (sprite != null) {
    ctx.drawImage(sprite, egg.position.x, egg.position.y, egg.width, egg.height);
  }
};
const renderLarva = (ctx, game, larva) => {
  ...
  if (sprite != null) {
    if (larva.caste == 'SUB_MINIMA') {
      for (let x = 0; x <= 1; x++) {
        for (let y = 0; y <= 1; y++) {
          scale = 0.5;
          ctx.drawImage(sprite, 32 * (Math.floor((game.time + larva.id % 10) / 12) % 3), 0, 32, 32, larva.position.x + x / 2, larva.position.y + y / 2, larva.width * scale, larva.height * scale);
        }
      }
    } else {
      ctx.drawImage(sprite, 32 * (Math.floor((game.time + larva.id % 10) / 12) % 3), 0, 32, 32, larva.position.x, larva.position.y, larva.width * scale, larva.height * scale);
    }
  }
  ...
  if (larva.caste != null && need != null && larva.playerID != null) {
    ...
    if (need < totalNeed) {
      ...
      ctx.arc(larva.position.x + larva.width * scale / 2,
      // x
      larva.position.y + larva.height * scale / 2,
      // y
      larva.width * 0.5,
      // radius
      Math.PI * 2 * (need / totalNeed) + Math.PI / 2, Math.PI / 2);
      ctx.stroke();
      ctx.restore();
    }
  }
};
const renderPupa = (ctx, game, pupa) => {
  let scale = 1;
  let num = 0;
  ...
  for (let x = 0; x <= num; x++) {
    for (let y = 0; y <= num; y++) {
      if (pupa.age < 40 && !pupa.isToken) {
        ...
        ctx.drawImage(sprite, 32 * 5 + 32 * Math.floor(pupa.age / 10), 0, 32, 32, pupa.position.x + x / 2, pupa.position.y + y / 2, pupa.width * scale, pupa.height * scale);
        return;
      }
      ...
      if (sprite != null) {
        if (pupa.caste != 'SUB_MINIMA') {
          ctx.save();
          ctx.translate(pupa.position.x + pupa.width / 2, pupa.position.y + pupa.height / 2);
          const rotation = [0, Math.PI / 2, Math.PI, Math.PI * 1.5][pupa.id % 4];
          ctx.rotate(rotation);
          ctx.drawImage(sprite, -pupa.width / 2, -pupa.height / 2, pupa.width, pupa.height);
          ctx.restore();
        } else {
          ctx.drawImage(sprite, pupa.position.x + x / 2, pupa.position.y + y / 2, pupa.width * scale, pupa.height * scale);
        }
      }
    }
  }
};
const renderToken = (ctx, game, token, isHeld) => {
  let renderFn = null;
  let renderType = null;
  let scale = 0.5;
  ctx.save();
  ...
  switch (token.pheromoneType) {
    ...
    case 'DOMESTICATE':
      ...
      renderFn = (ctx, game, beetle) => {
        ...
        ctx.drawImage(sprite.img, sprite.x, sprite.y, sprite.width, sprite.height, 0, 0, 1, 1);
      };
      scale = 0.8;
      break;
  }
...
const renderVine = (ctx, game, vine) => {
  ctx.save();
  ctx.translate(vine.position.x + vine.width / 2, vine.position.y + vine.height / 2);
  ...
  switch (vine.subType) {
    ...
    case 'STALK':
      ...
      ctx.drawImage(obj.img, obj.x, obj.y, obj.width, obj.height, 0, 0, vine.width * 1, vine.height * 1);
      ...
      for (const sprout of vine.sprouts) {
        ...
        // translation
        const vec = rotate(multiply({
          x: -1,
          y: 1
        }, sprout.pos), sprout.theta);
...
const renderSeed = (ctx, game, seed) => {
  const sprite = game.sprites.SEED;
  if (sprite != null) {
    ctx.drawImage(sprite, seed.position.x, seed.position.y, seed.width, seed.height);
  }
};
module.exports = {
  renderFood,
