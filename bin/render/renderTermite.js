...
const renderTermiteEgg = (ctx, game, entity) => {
  const egg = entity;
...
const renderDeadTermite = (ctx, game, entity) => {
  const termite = entity;
  ctx.save();
  // render relative to top left of grid square,
  // but first translate for rotation around the center
  ctx.translate(termite.position.x + termite.width / 2, termite.position.y + termite.height / 2);
  ...
  if (sprite.img != null) {
    ctx.drawImage(sprite.img, sprite.x, sprite.y, sprite.width, sprite.height, 0, 0, termite.width, termite.height);
  }
  ctx.restore();
};
const renderTermite = (ctx, game, entity) => {
  const termite = entity;
  ctx.save();
  // render relative to top left of grid square,
  // but first translate for rotation around the center
  ...
  ctx.translate(termite.position.x + termite.width / 2 + offset.x, termite.position.y + termite.height / 2 + offset.y);
  ctx.rotate(termite.theta - Math.PI / 2);
  ctx.translate(-(termite.width / 2 + offset.x), -(termite.height / 2 + offset.y));
  const sprite = getTermiteSprite(game, termite);
  if (sprite.img != null) {
    ctx.drawImage(sprite.img, sprite.x, sprite.y, sprite.width, sprite.height, 0, spriteOffset, termite.width, termite.height);
  }
  ...
  if (Math.ceil(termite.hp) < game.config[termite.playerID][termite.caste].hp) {
    renderHealthBar(ctx, {
      ...termite
    }, game.config[termite.playerID][termite.caste].hp);
  }
  ctx.restore();
