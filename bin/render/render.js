...
const renderView = (canvas, ctx2d, game, dims, isMini) => {
  ...
  if (game.viewImage.canvas != null) {
    if (isMini) {
      ctx.drawImage(game.viewImage.canvas,
      // canvas true dimensions:
      dims.viewPos.x / px, dims.viewPos.y / pxy, dims.viewWidth / px, dims.viewHeight / pxy,
      // minimap dimensions:
      dims.viewPos.x, dims.viewPos.y, dims.viewWidth, dims.viewHeight);
    } else {
      ctx.drawImage(game.viewImage.canvas, 0, 0, game.gridWidth, game.gridHeight);
    }
  } else {
    // background
  ...
  for (const id of game.ANT) {
    ...
    if (game.entities[id] != null && game.entities[id].caste == 'QUEEN' && game.queens[game.entities[id].playerID] == id) {
      continue;
    }
  ...
  if (queen != null && game.tickInterval != null) {
    ...
    if (queenAction.type == 'PUTDOWN') {
      const putdownPos = getPositionsInFront(game, queen).find(pos => {
        return lookupInGrid(game.grid, pos).map(id => game.entities[id]).filter(e => e.type.slice(0, 4) != 'DEAD' && e.type != 'BACKGROUND').length == 0;
      });
      if (putdownPos != null) {
  ...
  // marquee
  if (game.isExperimental && document.onmousemove != null && (game.mouse.isLeftDown || game.keepMarquee)) {
...
const refreshStaleImage = (game, dims) => {
  ...
  if (game.viewImage.allStale) {
    ...
    for (const posKey in game.viewImage.stalePositions) {
      ...
      if (obj != null && obj.img != null) {
        ctx.globalAlpha = 0.2;
        ctx.drawImage(obj.img, obj.x, obj.y, obj.width, obj.height, pos.x, pos.y, 1, 1);
        ctx.globalAlpha = 1;
      }
...
const renderEntity = (ctx, game, id, renderFn, alwaysOnScreen) => {
  let entity = game.entities[id];
  if (entity == null || entity.position == null) return;
  if (!onScreen(game, entity) && !game.config.unanimatedTypes.includes(entity.type) && !alwaysOnScreen && entity.type != 'FOREGROUND' && (entity.type != 'FOOT' && !entity.segmented || game.maxMinimap)) {
    return;
  }
  if (entity.type == 'ANT' || entity.type == 'TERMITE' || game.config.critterTypes.includes(entity.type)) {
...
const renderPheromones = (ctx, game) => {
  const config = game.config;
  const {
    grid
  } = game;
  for (let x = Math.max(0, Math.floor(game.viewPos.x)); x < Math.min(game.viewPos.x + game.viewWidth, game.gridWidth); x++) {
    for (let y = Math.max(0, Math.floor(game.viewPos.y)); y < Math.min(game.viewPos.y + game.viewHeight, game.gridHeight); y++) {
      if (!onScreen(game, {
        position: {
          x,
          y
      ...
      for (const playerID in game.players) {
        ...
        for (const pheromoneType in pheromonesAtCell) {
          ...
          if (game.showPheromoneValues) {
            ...
            const obj = getPheromoneSprite(game, {
              x,
              y
            }, player.id, pheromoneType);
            if (obj.img != null) {
              ctx.save();
              ctx.translate(x + 0.5, y + 0.5);
              ctx.rotate(obj.theta);
              ctx.drawImage(obj.img, obj.x, obj.y, obj.width, obj.height, -0.5, -0.5, 1, 1);
              ctx.restore();
            } else {
