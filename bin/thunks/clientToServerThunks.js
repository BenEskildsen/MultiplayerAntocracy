...
const registerSession = (state, ending) => {
  ...
  const queens = game.ANT.map(id => game.entities[id]).filter(a => a.playerID == game.playerID && a.caste == 'YOUNG_QUEEN').length;
  const ants = game.ANT.map(id => game.entities[id]).filter(a => a.playerID == game.playerID).length;
...
// ------------------------------------------------------------------
// Get Highscores
const getHighScores = params => {
  return axiosInstance.get('highscores', {
    params
  });
};
