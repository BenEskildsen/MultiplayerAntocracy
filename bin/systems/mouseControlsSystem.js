...
// Scroll
////////////////////////////////////////////////////////////////////
const onScroll = (store, ev, handlers) => {
...
const onMouseDown = (store, ev, handlers) => {
...
const onMouseUp = (store, ev, handlers) => {
  const mouseData = validateMouse(store, ev);
  if (mouseData == null) return;
  const {
    gridPos,
...
// Mouse move
////////////////////////////////////////////////////////////////////////////
const moveHandler = (store, handlers, ev) => {
  let canvas = document.getElementById('canvas');
  const {
    dispatch
  } = store;
...
const getMousePixel = (ev, canvas) => {
  ...
  if (ev.type === 'touchstart' || ev.type === 'touchmove' || ev.type === 'touchend') {
    ...
    if (!touch) return null; // NOTE: this is happening on touch end now?
    x = touch.clientX;
    y = touch.clientY;
  }
  const canvasPos = {
